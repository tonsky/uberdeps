...
```clojure
{:deps {uberdeps/uberdeps {:mvn/version "1.1.3"}}}
```
...
{ :paths ["src"]
  ...
  :aliases {
    :uberdeps {
      :replace-deps {uberdeps/uberdeps {:mvn/version "1.1.3"}}
      :replace-paths []
...
```sh
clj -Sdeps '{:aliases {:uberjar {:replace-deps {uberdeps/uberdeps {:mvn/version "1.1.3"}} :replace-paths []}}}' -M:uberjar -m uberdeps.uberjar
```
